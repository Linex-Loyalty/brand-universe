...
3. **Solo se agregan estrellas, y cada una brilla por su cuenta.** El número y
   la identidad de las estrellas es deliberadamente abierto: una categoría
   puede desprenderse en su propia estrella cuando el negocio lo justifique.
...
Linex Loyalty viene de **UltraGroup** y es el respaldo formal del ecosistema.
Firma solo en documentos formales, contratos y comunicación corporativa —
nunca dos marcas firmando con el mismo peso en una pieza comercial.
...
Linex Go y Linex Trip **no tienen genio**: son sub-marcas de canal dentro de
Travel, y el genio es de la estrella.
...
- La librería de íconos y la escala de radios *(unificadas el 16-sep-2026)*
...
- **No se mezclan los sistemas:** Trip no usa el color ni el tono de Go, ni al
  revés. Comparten tipografía, íconos y grilla porque eso es del grupo; el
  acento es de cada una.
- **Nunca compiten por la misma palabra clave.** Go rankea lo que busca una
  agencia ("sistema para agencias de viajes"), Trip lo que busca un viajero
  ("vuelos a San Andrés").
...
Consulta `pendientes` en `brand-tokens.json` para el estado vivo. Lo que más
pesa:
...
- **Linex Travel está peor documentada que sus dos sub-marcas.** Es la
  estrella de prioridad 1, está operando, y no tiene logo, paleta, voz ni
  manual propio.
- **Capital y School** no aparecen en ningún contexto de marca.
- **Trip arrastra deuda de contraste** del cambio de color del 14 de
  septiembre: tres ratios de su manual siguen medidos contra el Índigo
  anterior. Ninguna regla se rompe, pero los números están viejos.
