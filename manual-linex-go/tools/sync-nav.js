#!/usr/bin/env node
/* La navegación del manual de Linex Go, desde una sola definición.
 *
 * POR QUÉ EXISTE
...
/* El índice es el mismo del manual de Linex Trip, por decisión del administrador
   de marca (2026-09-15): las mismas quince secciones, en los mismos tres bloques
   y en el mismo orden, para que quien conoce un manual del grupo sepa moverse en
   el otro sin volver a aprenderlo.

   Go venía de 23. Seis se fusionaron por parejas donde Trip resuelve en una sola
   sección, y cinco se retiraron porque esta estructura no tiene dónde ponerlas
   (identidad fundamental, marcas representadas, uso de IA, reglas para agentes y
   gobernanza). Quedan en el historial de git.

   La única que no calca a Trip es la 02: allá son "Propuestas de logo" porque su
   isotipo está sin decidir; aquí es "Estado del logotipo", porque el de Go ya
   está decidido y lo que falta son catorce de las quince piezas del sistema. */
const BLOQUES = [
  {
